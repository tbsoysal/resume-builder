...
import {CvDataType, ProfileType, LanguageType, EducationType, SkillType, DocumentType, ExperienceType, CategoryName, CategoryTypes } from './CvData'; 
...
function Form({ cvData, setCvData }: FormProps) {
  const [profileVisibility, setProfileVisibility] = useState<ProfileType>({ visible: false });
  const [languageInput, setLanguageInput] = useState<LanguageType>({ name: '', level: 1, visible: false });
  const [documentInput, setDocumentInput] = useState<DocumentType>({ origin: '', name: '', date: '', visible: false });
  const [educationInput, setEducationInput] = useState<EducationType>({ program: '', university: '', date: '', visible: false });
  const [skillInput, setSkillInput] = useState<SkillType>({ text: '', visible: false });
  const [experienceInput, setExperienceInput] = useState<ExperienceType>({ companyName: '', jobName: '', jobDate: '', jobExplanation: '', visible: false });

  const addItem = (categoryName: CategoryName, newItemObject: CategoryTypes, resetInput: ()=>void) => {
  ...
  const removeItem = (categoryName: CategoryName, index: number) => {
    setCvData(prev => ({ ...prev, [categoryName]: prev[categoryName].filter((_, i) => i !== index) }))
  }

  const toggleVisibility = <T extends {visible?: boolean}>(toggler: React.Dispatch<React.SetStateAction<T>>) => {
